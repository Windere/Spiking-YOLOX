...
class AnnotationTransform(object):
    ...
    def __init__(self, class_to_ind=None, keep_difficult=True):
        self.class_to_ind = class_to_ind or dict(
            zip(FRUIT_CLASSES, range(len(FRUIT_CLASSES)))
        )
...
class FruitDetection(CacheDataset):
    ...
    def __init__(
        ...
        self._classes = FRUIT_CLASSES
        self.cats = [
            {"id": idx, "name": val} for idx, val in enumerate(FRUIT_CLASSES)
        ]
        self.class_ids = list(range(len(FRUIT_CLASSES)))
        self.ids = list()
    ...
    def _write_voc_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(FRUIT_CLASSES):
            cls_ind = cls_ind
